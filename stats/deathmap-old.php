<?php
  ...
  $words = array("alter","analyzer","backup","cache","change","check","checksum","commit","create","delete","describe","do","drop","explain","flush","grant","handler","insert","join","kill","load","lock","optimize","purge","rename","repair","replace","reset","restore","revoke","rollback","savepoint","select","set","show","start","stop","truncate","union","unlock","use");
    ...
    $db = new database();
    $db->query("SELECT COUNT(coord) as number, coord, INSTR(coord,',') as len FROM `tbl_death` WHERE tod >= DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL -$days DAY) AND coord LIKE '%, $zlevel' $class_str $antag_str $key_str GROUP BY coord ORDER BY len, coord");
    $db->execute();
    $data = $db->resultset();
    ...
    $i = 0;
    foreach($data as $d){
      $coord_tab = explode(", ",$d->coord);
      $x_d = $coord_tab[0];
      $y_d = $coord_tab[1];
      $deaths_tab[$x_d][$y_d] = $d->number;
      if($max_death_per_tile < $d->number){
        $max_death_per_tile = $d->number;
      }
      $sum_death += $d->number;
      $i++;
    }
    ...
    imagepng($image,DMEDIR."/icons/minimaps/Box Station_1.png");
    imagedestroy($image);
    
    print "<img src='".APP_URL."tgstation/icons/minimaps/Box Station_1.png'>";
